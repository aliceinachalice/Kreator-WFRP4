...
class Postac:
    ...
    rzem = (0, 'Rzemiosło (Dowolne)',  rapt, False, rbal, False, rgrb, False, rgot, False, rkal, False, rkow, False, rswi, False, rgar, False, rkam, False, rtru, False)  # (ilość użyć, nazwa, elementy) # temp
    ...
    def grupy(self, um):
        ...
        for x in um:
            if type(x) is int:
                ...
                if x:
                    licznik += 1
        return print(licznik)
        # rzem = (0, 'Rzemiosło (Dowolne)', rapt, rbal, rgrb, rgot, rkal, rkow, rswi, rgar, rkam, rtru)
        # Zapętlić z rozwojem umiejętności?
        """
        rzem = [self.rapt, True, self.rbal, True, self.rgrb, True]
        #   self.rgot, self.rkal, self.rkow, self.rswi, self.rgar, self.rkam, self.rtru]
        # rzem = (0, 'Rzemiosło (Dowolne)', rapt, rbal, rgrb, rgot, rkal, rkow, rswi, rgar, rkam, rtru)

        wyb_grup = {'rzemiosło': rzem}
        if um in wyb_grup:
            print(gdzie())
        return type(rzem)
"""
    """
    Rozwój umiejętności 
    sprawdza profesję (jako nazwę funkcji - nazwa wyświetlana w profesji)
    pobiera aktualny poziom postaci (w Postac)
    sprawdza czy jest w grupie rozwoju
    jeśli tak dodaje do listy wyboru?
    może powinno pobierać listę z poziomu i łączyć w całość?
...
def akceptacja(pytanie=' Akceptujesz wynik rzutu? (T/N)', *args):
    """(pytanie na które ma odpowiedzieć użytkownik, dopuszczalne znaki odpowiedzi)"""
    ...
    while licz <= len(lista):
        ...
        while (licz % 8) != 0:  # stała określająca liczbę profesji w pakiecie
            if licz > len(lista):  # jeśli nie ma więcej elementów w liście
                tresc += '\n '
                break
            tresc += f' {licz:02d}. {lista[licz - 1].nazwa[0]:18}'
    ...
    while True:
        ...
        while licz <= len(um):
            ...
            if wybrany == 'n':  # nastęna grupa umiejętności
                ...
                if grupa:  # dla umiejętności grupowych
                    p.grupy(um[int(wybrany) - 1])
                else:
...
if p.rasa == rasy[0]:  # modyfikator rasowy, PP, PB, Szybkość
    ...
    p.szyb = 3
    p_dodat = 2
    # Broń Biała (Podstawowa), Język (Khazalid), Odporność, Opanowanie, Mocna Głowa, Rzemiosło (Dowolne), Wiedza
    # (Geologia), Wiedza (Krasnoludy), Wiedza (Metalurgia), Wycena, Występy (Gawędziarstwo), Zastraszanie
    # umiej = [p.bbia, p.jkrs, p.odpo, p.opan, p.mglo, p.rapt, p.rbal, p.rgrb, p.rgot, p.rkal, p.rkow, p.rswi,
    # p.rgar, p.rkam, p.wgeo, p.wkrs, p.wmet, p.wycn, p.wsga, p.zast]
